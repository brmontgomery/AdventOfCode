...
{
	uint32_t returnVal = (uint32_t)input[3] & 0x000000ff;
	returnVal |= ((uint32_t)input[2] & 0x000000ff) << 8;
	returnVal |= ((uint32_t)input[1] & 0x000000ff) << 16;
	returnVal |= ((uint32_t)input[0] & 0x000000ff) << 24;
...
void MD5::runMD5Algorithm() {
	...
	for (int i = 0; i < 64; i++) {
		...
		d = c;
		c = b;
		uint32_t ratating = (a + f + k_array_[i] + workingBits[g]);
		uint32_t rotated = leftRotate(ratating, s_array_[i]);
		b = b + rotated;
		a = dTemp;

		std::cout << std::hex << a << std::endl << b << std::endl << c << std::endl << d << std::endl << std::endl;
	}
	...
	std::cout << a0_ << std::endl << b0_ << std::endl << c0_ << std::endl << d0_ << std::endl << std::endl;
}
...
std::string MD5::convertOutputToHexString() {
	std::cout << std::hex << a0_ << b0_ << c0_ << d0_ << std::endl;
	return "";
}